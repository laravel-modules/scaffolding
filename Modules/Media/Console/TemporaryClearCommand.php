...
class TemporaryClearCommand extends Command
{
    ...
    {
        TemporaryFile::whereDate('created_at', '<=', today()->subHours(6))->each(function (TemporaryFile $file) {
            $file->delete();
        });

        $this->info("The temporary files has been cleaned successfully. ". now()->toDateTimeString());
    }
}
